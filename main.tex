...
\maketitle
\begin{abstract}
The adrenal glands are two small organs that are one of the main components responsible to maintain the
homeostasis of the human body in the presence of chronic stressors. These glands
can be affected by a wide variety of lesions, both malignant and benign. The
most common adrenal lesions are the adenomas. Adrenal
radiologic evaluation via conventional imaging is a complex process that depends largely on the
experience and knowledge of the radiologist. The presence of pseudolesions, the
overlap of imaging features of different lesions or the incorrect choice of
imaging technique can lead to misdiagnosis, which can lead to increased
treatment costs or unnecessary examination since most of the adrenal lesions do
not require treatment. Machine learning methods have been proposed as a potential solution to
improve the diagnosis of adrenal lesions. In this paper we are focusing on the
analysis of research works that aim to diagnose/differentiate adrenal lesions
using conventional imaging techniques, specifically, magnetic resonance and
computed tomography, using machine learning methods. We divide our analysis in
three groups: A contains all the studies that focus on the differentiation
between adrenal adenomas and other adrenal lesions; B contains all the studies
that target the differentiation between benign and malignant lesions; C contains
the remaining studies, with dispersed focus. In total, we have analysed 23
studies in terms of the following categories: used dataset, modeling details and
evaluation metrics. We identified a lack of deep learning studies in this area,
a lack of prospective studies, multicenter validations and comparison between
the model diagnostic performance and the radiologist. Nevertheless, the
traditional machine learning models analysis imaging features (radiomics) have
achieved outstanding results when diagnosing adrenal lesions.
...
non-adenomas \cite{Platzek2019}, to avoid incurring in unnecessay treatment.
...
knowledge of the radiologist \cite{Zhang2022}. Several pitfalls can result in
misdiagnosis of adrenal lesions, such as the presence of pseudolesions, overlap
...
not only to improve the diagnostic accuracy of adrenal lesions, but also to
increase its efficiency. Most of these machine learning methods analyse medical
...
the studies range from 2017 until September of 2022.
...
presented data sets are unbalanced with much higher number of adenomas.
...
Selection Operator (LASSO). These studies achieved the best results of this
...
feature engineering, and the lack of development of deep learning method are

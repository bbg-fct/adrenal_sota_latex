...
This group corresponds to 52\% of all analysed studies and contains studies that
aim at the differentiation between adrenal adenomas and other lesions. Tables
\ref{tab:data_A,tab:model_A,tab:res_A} present an overview of the data sets,
models, and results, respectively, for each study inside group A.
\cite{Schieda2017,Tu2018,Tu2020} have data sets with adenomas and metastases.
\cite{Yi20181,Yi2018, Liu2022,Liu2021} have patients with adenomas and
pheochromocytomas. \cite{Elmohr2019, Torresan2021, Ho2019} have compared
adenomas and carcinomas. Finally, \cite{Romeo2018, Kusunoki2022} do not have
another specific lesion, \cite{Romeo2018} data set has 3 classes: lipid-poor
adenomas, lipid-rich adenoma and nonadenomas and \cite{Kusunoki2022} performs a
general differentiation between adenomas vs nonadenomas. In terms of the imaging
...
analyse MRI images focus on chemical shift images as they pose an remarkable
advantage in the detection of the lipip content of the lesions. All studies but
one performed lesion classification with ML models using radiomic features. The
most common model is the logistic regression (LogReg), followed by support
vector machine (SVM) and decision tree-based models. In every study, the region
of interest (ROI) was manually selected by experts. The extraction of first and
second-order statistics is a common practice, but only 3 studies extracted
shape-based features, and none extracted higher-order statistics. The only study
that deployed a DL model has performed classification of the ROI (selected,
cropped and labelled by experts) with a deep convolution neural network
\cite{Kusunoki2022}. They report the usage of augmentation techniques, such as
